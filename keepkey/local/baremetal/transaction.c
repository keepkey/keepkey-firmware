                   ...
                   TxOutputBinType *out, bool needs_confirm)
{
    ...
    if(in->script_type == OutputScriptType_PAYTOADDRESS)
    {
        ...
        else if(in->has_address)     // address provided -> regular output
        {
            if(needs_confirm)
            {
                coin_amnt_to_str(coin, in->amount, amount_str, sizeof(amount_str));
...
uint32_t compile_script_multisig_hash(const MultisigRedeemScriptType *multisig,
                                      uint8_t *hash)
{
    if(!multisig->has_m) { return 0; }
...
uint32_t serialize_script_sig(const uint8_t *signature, uint32_t signature_len,
                              const uint8_t *pubkey, uint32_t pubkey_len, uint8_t *out)
{
    uint32_t r = 0;
...
uint32_t tx_serialize_input(TxStruct *tx, const TxInputType *input, uint8_t *out)
{
    ...
    memcpy(out + r, input->script_sig.bytes, input->script_sig.size);
...
uint32_t tx_serialize_input_hash(TxStruct *tx, const TxInputType *input)
{
    ...
    sha256_Update(&(tx->ctx), input->script_sig.bytes, input->script_sig.size);
    r += input->script_sig.size;
...
uint32_t tx_serialize_output(TxStruct *tx, const TxOutputBinType *output, uint8_t *out)
{
    ...
    memcpy(out + r, output->script_pubkey.bytes, output->script_pubkey.size);
...
void tx_init(TxStruct *tx, uint32_t inputs_len, uint32_t outputs_len, uint32_t version,
             uint32_t lock_time, bool add_hash_type)
{

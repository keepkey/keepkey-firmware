    ...
    switch(signing_stage)
    {
        ...
        case STAGE_REQUEST_4_INPUT:
            ...
            if(idx2 == idx1)
            {
                ...
                if(hdnode_private_ckd_cached(&node, tx->inputs[0].address_n,
                                             tx->inputs[0].address_n_count) == 0)
                {
                ...
                if(tx->inputs[0].script_type == InputScriptType_SPENDMULTISIG)
                {
                    ...
                    tx->inputs[0].script_sig.size = compile_script_multisig(&(tx->inputs[0].multisig),
                                                    tx->inputs[0].script_sig.bytes);
                }
                else     // SPENDADDRESS
                {
                    ecdsa_get_pubkeyhash(node.public_key, hash);
                    tx->inputs[0].script_sig.size = compile_script_sig(coin->address_type, hash,
                                                    tx->inputs[0].script_sig.bytes);
                }
        ...
        case STAGE_REQUEST_4_OUTPUT:
            ...
            else
            {
                ...
                if(input.script_type == InputScriptType_SPENDMULTISIG)
                {
                    ...
                    memcpy(input.multisig.signatures[pubkey_idx].bytes, resp.serialized.signature.bytes,
                           resp.serialized.signature.size);
                    input.multisig.signatures[pubkey_idx].size = resp.serialized.signature.size;
                    input.script_sig.size = serialize_script_multisig(&(input.multisig),
                ...
                else     // SPENDADDRESS
                {
                    input.script_sig.size = serialize_script_sig(resp.serialized.signature.bytes,
                                            resp.serialized.signature.size, pubkey, 33, input.script_sig.bytes);
                }

                resp.serialized.serialized_tx.size = tx_serialize_input(&to, &input,
        ...
        case STAGE_REQUEST_5_OUTPUT:
            ...
            resp.has_serialized = true;
            resp.serialized.has_serialized_tx = true;
            resp.serialized.serialized_tx.size = tx_serialize_output(&to, &bin_output,
                                                 resp.serialized.serialized_tx.bytes);

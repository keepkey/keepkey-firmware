...
static bool rng_health_draw_bounded(uint8_t* buf, size_t len) {
  uint32_t word = 0;
  for (size_t i = 0; i < len; i++) {
    if (i % 4 == 0 && !random32_bounded(&word, 100000)) {
      memzero(buf, len);
      return false;
    }
...
static bool rng_health_gate(void) {
  ...
  for (size_t drawn = 0; drawn < RNG_HEALTH_SAMPLE_BYTES;
       drawn += sizeof(chunk)) {
    if (!rng_health_draw_bounded(chunk, sizeof(chunk))) {
...
bool random_buffer_checked(uint8_t* buf, size_t len) {
  ...
  if (!rng_health_draw_bounded(buf, len)) {
    rng_verdict = RNG_FAILED;
    return false;
  }
#ifdef EMULATOR

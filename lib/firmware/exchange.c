...
/*
 * exchange_tx_layout_str() - assemble display message for exchange transaction
 * output
 *
 * INPUT
 ...
 *
 */
static bool exchange_tx_layout_str(const CoinType *coin, const uint8_t *amt,
                                   size_t amt_len, char *out, size_t out_len) {
  const TokenType *token = NULL;
  if (!isEthereumLike(coin->coin_name) && coin->has_contract_address) {
    if (!coin->has_forkid) return false;
...
/// for ETH-like addresses.
bool addresses_same(const char *LHS, size_t LHS_len, const char *RHS,
                    size_t RHS_len, bool isETH) {
  if (isETH) {
...
/*
 * verify_exchange_address - verify address specified in exchange contract
 * belongs to device.
 *
 * INPUT
 *     coin - the CoinType
 ...
 */
static bool verify_exchange_address(
    const CoinType *coin, size_t address_n_count, const uint32_t *address_n,
    bool has_script_type, InputScriptType script_type, const char *address_str,
    size_t address_str_len, const HDNode *root, bool is_token) {
  ...
  if (isEthereumLike(coin->coin_name) || is_token) {
    ...
    return addresses_same(tx_out_address, sizeof(tx_out_address), address_str,
                          address_str_len, true);
  }
  ...
  if (!has_script_type) script_type = InputScriptType_SPENDADDRESS;
...
bool verify_exchange_coin(const char *coin1, const char *coin2, uint32_t len) {
  if (strncasecmp(coin1, coin2, len) == 0) return true;

  const CoinType *response_coin = coinByNameOrTicker(coin2);
  if (!response_coin) return false;

  if (strncasecmp(coin1, response_coin->coin_shortcut, len) == 0) return true;
...
static const CoinType *getWithdrawCoin(const ExchangeType *exchange) {
  const CoinType *coin =
      coinByNameOrTicker(exchange->signed_exchange_response.responseV2
                             .withdrawal_address.coin_type);

  if (!coin) coin = coinByNameOrTicker(exchange->withdrawal_coin_name);

  return coin;
}

static const CoinType *getDepositCoin(const ExchangeType *exchange) {
  return coinByNameOrTicker(
...
/*
 * verify_exchange_dep_amount() Verify deposit amount specified in exchange
 * contract
 *
 * INPUT
...
static bool verify_exchange_dep_amount(
  ...
  if (isEthereumLike(coin)) {
    ...
    if (exch_dep_amt->size > sizeof(uint64_t)) return false;
...
static bool verify_coins_match(const CoinType *lhs, const CoinType *rhs) {
  ...
  if (strcasecmp(lhs->coin_shortcut, rhs->coin_shortcut) == 0) return true;

  if (strcasecmp(lhs->coin_name, rhs->coin_name) == 0) return true;
 ...
 *     true/false -  success/failure
 */
static bool verify_exchange_contract(const CoinType *coin, void *vtx_out,
                                     const HDNode *root) {
  ...
  if (isEthereumLike(coin->coin_name)) {
    ...
    if (ethereum_isStandardERC20Transfer(tx_out)) {
      if (!ethereum_getStandardERC20Recipient(tx_out, tx_out_address,
                                              sizeof(tx_out_address)) ||
    ...
    memcpy(tx_out_address, tx_out->outputs[0].address,
           sizeof(tx_out->outputs[0].address));
  ...
  int response_raw_filled_len =
      encode_pb((const void *)&exchange->signed_exchange_response.responseV2,
                ExchangeResponseV2_fields, response_raw, sizeof(response_raw));
...
#if DEBUG_LINK
  if (memcmp(exchange->signed_exchange_response.signature.bytes, "FAKE_SIG",
             sizeof("FAKE_SIG")) != 0 &&
      cryptoMessageVerify(
          signed_coin, response_raw, response_raw_filled_len, ShapeShift_pubkey,
          (uint8_t *)exchange->signed_exchange_response.signature.bytes) != 0)
#else
  if (cryptoMessageVerify(
          signed_coin, response_raw, response_raw_filled_len, ShapeShift_pubkey,
          (uint8_t *)exchange->signed_exchange_response.signature.bytes) != 0)
#endif
  {
  ...
  if (!addresses_same(
          ...
          sizeof(exchange->signed_exchange_response.responseV2.deposit_address
                     .address),
  ...
  if (!verify_exchange_dep_amount(
          coin->coin_name, tx_out_amount,
          &exchange->signed_exchange_response.responseV2.deposit_amount)) {
  ...
  if (withdraw_coin) {
    if (!verify_coins_match(
            withdraw_coin,
            coinByNameOrTicker(exchange->withdrawal_coin_name))) {
    ...
    if (exchange->withdrawal_address_n_count) {
      if (!verify_exchange_address(
              withdraw_coin, exchange->withdrawal_address_n_count,
              exchange->withdrawal_address_n,
              exchange->has_withdrawal_script_type,
              exchange->withdrawal_script_type,
              exchange->signed_exchange_response.responseV2.withdrawal_address
                  .address,
              sizeof(exchange->signed_exchange_response.responseV2
  ...
  if (!verify_exchange_address(
          return_coin, exchange->return_address_n_count,
          exchange->return_address_n, exchange->has_return_script_type,
          exchange->return_script_type,
          exchange->signed_exchange_response.responseV2.return_address.address,
          sizeof(exchange->signed_exchange_response.responseV2.return_address
                     .address),
 ...
 *     exchange error code
 */
ExchangeError get_exchange_error(void) { return (exchange_error); }

#if DEBUG_LINK
const char *get_exchange_msg(void) { return exchange_msg ? exchange_msg : ""; }
#endif

/*
 * process_exchange_contract() - validate contract from exchange and populate
 * the transaction output structure
 *
 * INPUT
 ...
 *      true/false - success/failure
 */
bool process_exchange_contract(const CoinType *coin, void *vtx_out,
                               const HDNode *root, bool needs_confirm) {
  /* validate contract before processing */
  if (!verify_exchange_contract(coin, vtx_out, root)) return false;

  /* check if user confirmation is required */
  if (!needs_confirm) return true;
  ...
  if (!exchange_tx_layout_str(
          ...
          amount_dep_str, sizeof(amount_dep_str))) {
    set_exchange_error(ERROR_EXCHANGE_DEPOSIT_AMOUNT);
    return false;
  }
  ...
  switch (tx_exchange->signed_exchange_response.responseV2.type) {
    case OrderType_Precise: {
      ...
      if (!tx_exchange->signed_exchange_response.responseV2
               .has_withdrawal_amount ||
          !exchange_tx_layout_str(withdraw_coin,
                                  tx_exchange->signed_exchange_response
                                      .responseV2.withdrawal_amount.bytes,
                                  tx_exchange->signed_exchange_response
                                      .responseV2.withdrawal_amount.size,
                                  amount_wit_str, sizeof(amount_wit_str))) {
    ...
    case OrderType_Quick: {
      strlcpy(amount_wit_str, withdraw_symbol, sizeof(amount_wit_str));
  ...
  if (withdraw_coin && tx_exchange->withdrawal_address_n_count) {
    ...
    if (!bip32_node_to_string(node_str, sizeof(node_str), withdraw_coin,
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ConfirmOutput, "Confirm",
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ConfirmOutput,
                 ...
                 withdraw->has_dest_tag
                     ? "ShapeShift will send the %s to:\n%s tag:%s"
                     : "ShapeShift will send the %s to:\n%s",
                 withdraw_symbol, withdraw->address, withdraw->dest_tag)) {
      set_exchange_error(ERROR_EXCHANGE_CANCEL);
      return false;
    }
  }

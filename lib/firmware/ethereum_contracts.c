...
bool ethereum_contractHandled(uint32_t data_total, const EthereumSignTx* msg,
  ...
  /* transformERC20's dynamic transformations tail can alter execution and is
   * not fully rendered. Keep it on the raw-calldata path until every element
   * is parsed and disclosed. */
  ...
  /* Legacy MakerDAO selectors are not sufficient identity: without a pinned
   * deployment address, an arbitrary contract can impersonate these calls.
   * Keep them on the raw-calldata path. */

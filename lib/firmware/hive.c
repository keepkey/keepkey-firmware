...
bool hive_getPublicKeys(const HDNode* root, uint32_t account_index,
  ...
  if (account_index & 0x80000000u) return false;
  ...
  for (uint32_t i = 0; i < op_count; i++) {
    ...
    switch (op_type) {
      ...
      case HIVE_OP_ACCOUNT_UPDATE2: {  // active or posting authority
        /* account_update2 always carries a memo key, even when its authority
         * optionals are absent. Without trusted chain state the device cannot
         * prove that hidden key is unchanged, so presenting this as only a
         * profile update would authorize an undisclosed key rotation. */
        return E_AUTHORITY;
      }

...
static const TokenType* pool_underlying_token(const EthereumSignTx* msg) {
  ...
  while (token_index >= 0) {
    ...
    /* tokenIter() returns UnknownToken *and* sets the counter to -1 when the
     * list is exhausted, so the body still runs once more unless we stop here.
     * UnknownToken.address is a 3-byte string literal; reading 20 bytes of it
     * is an over-read past the literal. */
...
/// \param[out] token_out When non-NULL, receives the pool's non-WETH token on
///     success. Lets the confirm path reuse the one table scan the shape check
///     already paid for, instead of walking all of tokens[] a second time.
...
bool zx_isZxApproveLiquid(const EthereumSignTx* msg) {
  return approve_shape_is_clear_signable(msg, NULL);
}

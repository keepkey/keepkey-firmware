...
bool thor_has_deposit_selector(const EthereumSignTx *msg) {
  ...
  return (memcmp(msg->data_initial_chunk.bytes, THOR_SELECTOR_DEPOSIT, 4) == 0 ||
          memcmp(msg->data_initial_chunk.bytes, THOR_SELECTOR_DEPOSIT_WITH_EXPIRY, 4) == 0);
}

bool thor_is_expiry_variant(const EthereumSignTx *msg) {
  if (msg->data_initial_chunk.size < 4) return false;
  return memcmp(msg->data_initial_chunk.bytes, THOR_SELECTOR_DEPOSIT_WITH_EXPIRY, 4) == 0;
}
...
bool thor_confirmThorTx(uint32_t data_total, const EthereumSignTx *msg) {
  ...
  thorchainData = (uint8_t *)(msg->data_initial_chunk.bytes + 4 +
                               (is_expiry ? 6 : 5) * 32);

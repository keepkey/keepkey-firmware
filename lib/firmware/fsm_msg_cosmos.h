...
static bool cosmos_formatAmount(uint64_t amount, char* out, size_t out_len) {
  if (bn_format_uint64(amount, NULL, " ATOM", 6, 0, false, out, out_len)) {
    return true;
  }
  tendermint_signAbort();
  fsm_sendFailure(FailureType_Failure_SyntaxError,
                  _("Amount cannot be displayed"));
  layoutHome();
  return false;
}
...
void fsm_msgCosmosMsgAck(const CosmosMsgAck* msg) {
  ...
  if (msg->has_send) {
    ...
    switch (msg->send.address_type) {
      ...
      default: {
        char amount_str[32];
        if (!cosmos_formatAmount(msg->send.amount, amount_str,
                                 sizeof(amount_str)))
          return;
    ...
    char amount_str[32];
    if (!cosmos_formatAmount(msg->delegate.amount, amount_str,
                             sizeof(amount_str)))
      return;
    ...
    /** Confirm transaction parameters on-screen */
    char amount_str[32];
    if (!cosmos_formatAmount(msg->undelegate.amount, amount_str,
                             sizeof(amount_str)))
      return;
    ...
    /** Confirm transaction parameters on-screen */
    char amount_str[32];
    if (!cosmos_formatAmount(msg->redelegate.amount, amount_str,
                             sizeof(amount_str)))
      return;
    ...
    if (msg->rewards.has_amount) {
      /** Confirm transaction parameters on-screen */
      char amount_str[32];
      if (!cosmos_formatAmount(msg->rewards.amount, amount_str,
                               sizeof(amount_str)))
        return;
    ...
    /** Confirm transaction parameters on-screen */
    char amount_str[32];
    if (!cosmos_formatAmount(msg->ibc_transfer.amount, amount_str,
                             sizeof(amount_str)))
      return;

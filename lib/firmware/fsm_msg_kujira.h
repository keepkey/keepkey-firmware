...
void fsm_msgKujiraGetAddress(const KujiraGetAddress *msg) {
  RESP_INIT(KujiraAddress);

  CHECK_INITIALIZED

  CHECK_PIN

  const CoinType *coin = fsm_getCoin(true, "Kujira");
  if (!coin) {
    return;
  }
  ...
  if (!tendermint_getAddress(node, "kujira", resp->address)) {
  ...
  memzero(node, sizeof(*node));
  msg_write(MessageType_MessageType_KujiraAddress, resp);
  layoutHome();
}
...
void fsm_msgKujiraSignTx(const KujiraSignTx *msg) {
  ...
  RESP_INIT(KujiraMsgRequest);

  if (!tendermint_signTxInit(node, (void *)msg, sizeof(KujiraSignTx),
                             "ukuji")) {
    tendermint_signAbort();
    memzero(node, sizeof(*node));
    fsm_sendFailure(FailureType_Failure_FirmwareError,
  ...
  memzero(node, sizeof(*node));
  msg_write(MessageType_MessageType_KujiraMsgRequest, resp);
  layoutHome();
}
...
void fsm_msgKujiraMsgAck(const KujiraMsgAck *msg) {
  ...
  const KujiraSignTx *sign_tx = (KujiraSignTx *)tendermint_getSignTx();
  ...
  if (msg->has_send) {
    ...
    switch (msg->send.address_type) {
      ...
      default: {
        char amount_str[32];
        bn_format_uint64(msg->send.amount, NULL, " KUJI", 6, 0, false,
    ...
    char amount_str[32];
    bn_format_uint64(msg->delegate.amount, NULL, " KUJI", 6, 0, false,
                     amount_str, sizeof(amount_str));

    if (!confirm_kujira_address("Confirm delegator address",
    ...
    if (!confirm_kujira_address("Confirm validator address",
    ...
    if (!tendermint_signTxUpdateMsgDelegate(
            ...
            msg->delegate.validator_address, "kujira", "ukuji", "cosmos-sdk")) {
      tendermint_signAbort();
      fsm_sendFailure(FailureType_Failure_SyntaxError,
    ...
    /** Confirm transaction parameters on-screen */
    char amount_str[32];
    bn_format_uint64(msg->undelegate.amount, NULL, " KUJI", 6, 0, false,
                     amount_str, sizeof(amount_str));
    ...
    if (!tendermint_signTxUpdateMsgUndelegate(
            ...
            msg->undelegate.validator_address, "kujira", "ukuji",
            "cosmos-sdk")) {
      tendermint_signAbort();
      fsm_sendFailure(FailureType_Failure_SyntaxError,
    ...
    /** Confirm transaction parameters on-screen */
    char amount_str[32];
    bn_format_uint64(msg->redelegate.amount, NULL, " KUJI", 6, 0, false,
                     amount_str, sizeof(amount_str));
    ...
    if (!confirm_kujira_address("Delegator address",
    ...
    if (!confirm_kujira_address("Validator source address",
    ...
    if (!confirm_kujira_address("Validator dest. address",
    ...
    if (!tendermint_signTxUpdateMsgRedelegate(
            ...
            msg->redelegate.validator_src_address,
            msg->redelegate.validator_dst_address, "kujira", "ukuji",
            "cosmos-sdk")) {
      tendermint_signAbort();
      fsm_sendFailure(FailureType_Failure_SyntaxError,
    ...
    if (msg->rewards.has_amount) {
      /** Confirm transaction parameters on-screen */
      char amount_str[32];
      bn_format_uint64(msg->rewards.amount, NULL, " KUJI", 6, 0, false,
                      amount_str, sizeof(amount_str));
    ...
    /** Confirm transaction parameters on-screen */
    char amount_str[32];
    bn_format_uint64(msg->ibc_transfer.amount, NULL, " KUJI", 6, 0, false,
                     amount_str, sizeof(amount_str));
    ...
    if (!tendermint_signTxUpdateMsgIBCTransfer(
            ...
            msg->ibc_transfer.revision_height, "kujira", "ukuji",
            "cosmos-sdk")) {
      tendermint_signAbort();
      fsm_sendFailure(FailureType_Failure_SyntaxError,
                      "Failed to include send message in transaction");
  ...
  if (!confirm(ButtonRequestType_ButtonRequest_SignTx, node_str,
               "Sign this Kujira transaction on %s? "
               "It includes a fee of %" PRIu32 " uKUJI and %" PRIu32 " gas.",
  ...
  tendermint_signAbort();
  layoutHome();
  msg_write(MessageType_MessageType_KujiraSignedTx, resp);
}

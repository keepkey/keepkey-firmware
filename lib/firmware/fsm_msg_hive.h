...
void fsm_msgHiveGetPublicKey(const HiveGetPublicKey* msg) {
  ...
  if (!hive_getPublicKey(node->public_key, public_key, sizeof(public_key))) {
  ...
  resp->raw_public_key.size = sizeof(raw_public_key);
  memcpy(resp->raw_public_key.bytes, raw_public_key, sizeof(raw_public_key));
  resp->has_public_key = true;
  strlcpy(resp->public_key, public_key, sizeof(resp->public_key));
  memzero(raw_public_key, sizeof(raw_public_key));
...
void fsm_msgHiveGetPublicKeys(const HiveGetPublicKeys* msg) {
  ...
  char owner_key[64], active_key[64], memo_key[64], posting_key[64];
  if (!hive_getPublicKeys(root, account_index, owner_key, sizeof(owner_key),
                          active_key, sizeof(active_key), memo_key,
                          sizeof(memo_key), posting_key, sizeof(posting_key))) {
  ...
  strlcpy(resp->owner_key, owner_key, sizeof(resp->owner_key));
  strlcpy(resp->active_key, active_key, sizeof(resp->active_key));
  strlcpy(resp->memo_key, memo_key, sizeof(resp->memo_key));
  strlcpy(resp->posting_key, posting_key, sizeof(resp->posting_key));
  memzero(owner_key, sizeof(owner_key));
  memzero(active_key, sizeof(active_key));
  memzero(memo_key, sizeof(memo_key));
  memzero(posting_key, sizeof(posting_key));

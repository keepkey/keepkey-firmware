...
bool mayachain_signTxUpdateMsgSend(const uint64_t amount,
  ...
  const char* pfix;
  /* coin_denom is streamed directly into the hash below, so this scratch
     buffer contains only bounded numeric and address fragments. The longest
     formatted address fragment is 63 bytes plus NUL; retain the existing

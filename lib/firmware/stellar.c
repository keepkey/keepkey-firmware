                                  ...
                                  const char *str_account)
{
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, _("Op src account OK?"), "%s\n%s\n%s", str_addr_rows[0], str_addr_rows[1], str_addr_rows[2]))
    {
...
bool stellar_confirmCreateAccountOp(const StellarCreateAccountOp *msg)
{
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, _("Create account: "), "%s\n%s\n%s", str_addr_rows[0], str_addr_rows[1], str_addr_rows[2]))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
...
bool stellar_confirmPaymentOp(const StellarPaymentOp *msg)
{
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_pay_amount, "%s\n%s\n%s\n%s", str_asset_row, str_to, str_addr_rows[1], str_addr_rows[2]))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
...
bool stellar_confirmPathPaymentOp(const StellarPathPaymentOp *msg)
{
  ...
  ....
  */
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_pay_amount, "%s\n%s\n%s\n%s", str_dest_asset, str_to, str_dest_rows[1], str_dest_rows[2]))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_source_amount, "%s\n%s\n%s", str_send_asset, _("This is the amount debited"), _("from your account.")))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
...
bool stellar_confirmManageOfferOp(const StellarManageOfferOp *msg)
{
  ...
  */
   ...
   */
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_offer, "%s\n%s\n%s\n%s", str_selling, str_selling_asset, str_buying, str_buying_asset))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
...
bool stellar_confirmSetOptionsOp(const StellarSetOptionsOp *msg)
{
    ...
    if (msg->has_signer_type)
    {
        ...
        if (msg->signer_type == 0)
        {
            ...
            if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_title, "%s\n%s\n%s\n%s", str_weight_row, str_addr_rows[0], str_addr_rows[1], str_addr_rows[2]))
            {
                stellar_signingAbort(_("User canceled"));
                return false;
            }
        ...
        if (msg->signer_type == 1)
        {
            ...
            if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_title, "%s\n%s", str_weight_row, _("(confirm hash on next screen)")))
            {
                stellar_signingAbort(_("User canceled"));
                return false;
            }
        ...
        if (needs_hash_confirm)
        {
            ...
            if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, _("Confirm Hash"), "%s\n%s\n%s\n%s", rows[0], rows[1], rows[2], rows[3]))
            {
                stellar_signingAbort(_("User canceled"));
                return false;
            }
...
bool stellar_confirmChangeTrustOp(const StellarChangeTrustOp *msg)
{
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_title, "%s\n%s\n%s\n%s", str_amount_row, str_addr_rows[0], str_addr_rows[1], str_addr_rows[2]))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
...
bool stellar_confirmAllowTrustOp(const StellarAllowTrustOp *msg)
{
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_title, "%s\n%s\n%s\n%s", str_asset_row, str_by, str_trustor_rows[1], str_trustor_rows[2]))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
...
bool stellar_confirmAccountMergeOp(const StellarAccountMergeOp *msg)
{
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, _("Merge Account"), "%s\n%s\n%s\n%s", _("All XLM will be sent to:"), str_destination_rows[0], str_destination_rows[1], str_destination_rows[2]))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
...
bool stellar_confirmManageDataOp(const StellarManageDataOp *msg)
{
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_title, "%s", msg->key))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
    ...
    if (msg->has_value)
    {
        ...
        if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_title, "%s", str_hash_digest))
        {
            stellar_signingAbort(_("User canceled"));
            return false;
        }
...
bool stellar_confirmBumpSequenceOp(const StellarBumpSequenceOp *msg)
{
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, _("Bump Sequence"), "%s\n%s", _("Set sequence to:"), str_bump_to))
    {
        stellar_signingAbort(_("User canceled"));
        return false;
    }
...
void stellar_signingAbort(const char *reason)
{
    ...
    fsm_sendFailure(FailureType_Failure_ActionCancelled, reason);
    layoutHome();
}
...
bool stellar_getAddressBytes(const char *str_address, uint8_t *out_bytes)
{
    ...
    if (!stellar_validateAddress(str_address))
        return false;
...
void stellar_layoutTransactionSummary(const StellarSignTx *msg)
{
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_lines[0], "%s\n%s\n%s\n%s", _("Signing with:"), str_addr_rows[0], str_addr_rows[1], str_addr_rows[2]))
    {
        stellar_signingAbort(_("User canceled"));
        return;
    }
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, str_lines[0], "%s\n%s\n%s\n%s", str_lines[1], str_lines[2], str_lines[3], str_lines[4]))
    {
        stellar_signingAbort(_("User canceled"));
        return;
    }
    ...
    if (msg->timebounds_start || msg->timebounds_end)
    {
        if (!confirm(ButtonRequestType_ButtonRequest_ProtectCall, _("Confirm Time Bounds"), "%s\n%s\n%s\n%s", str_lines[0], str_lines[1], str_lines[2], str_lines[3]))
        {
            stellar_signingAbort(_("User canceled"));
            return;
        }
...
bool stellar_get_address(const uint8_t *pubkey, const uint32_t version_byte, char *address, const size_t size)
{
    ...
    if (size < 57)
    { // 56 characters, 1 null termination byte
        return false;
    }

...
int kkemu_init(uint8_t* flash_buf, size_t flash_len) {
  ...
  /* A shared library can be initialized more than once in one host process.
   * Do not carry the prior wallet's auto-lock deadline into this session. */
  reset_idle_time();
...
static void kkemu_poll_body(void) {
  ...
  usbPoll();
  increment_idle_time(KKEMU_POLL_INTERVAL_MS);
  toggle_screensaver();
  animate();
  display_refresh();
}

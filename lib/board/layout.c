...
void layout_init(Canvas* new_canvas) {
  ...
  /* Emulator hosts can tear down and initialize another wallet without
   * unloading the dylib. Rebuild both intrusive queues from their backing
   * array; pushing the same nodes onto an old free list creates a cycle and
   * makes the next layout_clear_animations() loop forever. */
  active_queue = (AnimationQueue){NULL, 0};
  free_queue = (AnimationQueue){NULL, 0};
  animate_flag = false;

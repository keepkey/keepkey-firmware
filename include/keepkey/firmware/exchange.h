...
#if DEBUG_LINK
#define set_exchange_error(error_code) \
  set_exchange_errorDebug((error_code), __FILE__ ":" VERSTR(__LINE__) ":")
 ...
 * \param value         The token value interpreted as a 32byte big endian
 * number \param value_len     Length in bytes of value \param decimal       The
 * number of decimals to format this token amount to \param out_str       128
 * byte buffer to store string result \returns             boolean representing
 * wethere the token value was successfully formatted
 */
bool ether_token_for_display(const uint8_t *value, uint32_t value_len,
                             uint32_t decimal, char *out_str, size_t out_len);

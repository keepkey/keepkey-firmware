...
struct ScopedStorageFlash {
  ...
  ~ScopedStorageFlash() {
    // Some tests intentionally boot a bitcoin-only-band wallet, which sets a
    // process-wide lock latch. A real reboot starts with that latch clear;
    // erase this disposable image before restoring the prior flash so later
    // tests observe the same clean startup state.
    storage_wipe();
    storage_reset();
    emulator_flash_base = previous;
  }
  char *sector(Allocation location = FLASH_STORAGE1) {
    return reinterpret_cast<char *>(flash_write_helper(location));
  }
};

}  // namespace

TEST(Storage, ScopedFlashPreservesTheProcessFlash) {
  uint8_t *before = emulator_flash_base;
  uint8_t *initialized = nullptr;
  {
    ScopedStorageFlash flash;
    initialized = flash.previous;
    ASSERT_NE(nullptr, initialized);
    ASSERT_NE(initialized, emulator_flash_base);
  }
  EXPECT_EQ(before ? before : initialized, emulator_flash_base);
}
...
TEST(Storage, AdvancedModeIsNeverRestoredFromFlash) {
  ...
  struct {
    ...
    void (*read)(ConfigFlash *, const char *, size_t);
      ...
      {"V11", storage_readV11}, {"V16", storage_readV16},
      {"V17", storage_readV17}, {"V18", storage_readV18},
      {"V19", storage_readV19},
  };
...
TEST(Storage, AdvancedModeStaleBitIsCommittedOutOfRealFlash) {
  ScopedStorageFlash flash;
  char *active_before = flash.sector(storage_getLocation());
  uint32_t stale = 0;
  memcpy(&stale, active_before + 48, sizeof(stale));
  stale |= (1u << 12);
  memcpy(active_before + 48, &stale, sizeof(stale));

  storage_init();

  const char *active =
      reinterpret_cast<const char *>(flash_write_helper(storage_getLocation()));
  uint32_t persisted_flags = 0;
  memcpy(&persisted_flags, active + 48, sizeof(persisted_flags));
  EXPECT_EQ(0u, persisted_flags & (1u << 12));
...
TEST(Storage, BitcoinOnlyBandInitializationLeavesFlashUntouched) {
  ScopedStorageFlash flash;
  char *active = flash.sector(storage_getLocation());
  uint32_t version = STORAGE_VERSION_BTC_ONLY;
  memcpy(active + 44, &version, sizeof(version));
  ...
  // A process-lifetime emulator/dylib can now mount another ordinary image.
  // Its state must be derived from that image, not inherited from the prior
  // bitcoin-only mount.
...
TEST(Storage, VersionedWritersRejectShortBuffersWithoutWriting) {
  ...
  const size_t legacy_size =
      1501 + V17_ENCSEC_SIZE +
      PERSISTENT_IDENTITY_COUNT * (71 + CLEARSIGN_ICON_MAX);
  check(storage_writeStorageV18, legacy_size);
  check(storage_writeStorageV19, legacy_size);
}

TEST(Storage, V19WriterClearsAStalePinKdfFlag) {
  Storage storage = {};
  const size_t size = 1501 + V17_ENCSEC_SIZE +
                      PERSISTENT_IDENTITY_COUNT *
                          (71 + CLEARSIGN_ICON_MAX);
  std::vector<char> bytes(size, 0);
  uint32_t stale = 1u << 20;
  memcpy(bytes.data() + 4, &stale, sizeof(stale));

  storage.pub.pin_kdf_v2 = false;
  storage_writeStorageV19(bytes.data(), bytes.size(), &storage);

  uint32_t flags = 0;
  memcpy(&flags, bytes.data() + 4, sizeof(flags));
  EXPECT_EQ(0u, flags & (1u << 20));
}
...
TEST(Storage, FutureBitcoinBandInitializationLeavesFlashUntouched) {
  ScopedStorageFlash flash;
  char *active = flash.sector(storage_getLocation());
  uint32_t version = STORAGE_VERSION_BTC_ONLY + 1;
  memcpy(active + 44, &version, sizeof(version));
  const auto before = flash.bytes;

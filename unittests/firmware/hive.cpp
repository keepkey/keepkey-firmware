...
// account_update2 always carries a memo key. Without trusted chain state the
// device cannot distinguish an unchanged key from a hidden rotation, so it
// must not summarize any form of this operation as only a profile update.
TEST(Hive, AccountUpdate2IsRejectedBecauseMemoKeyIsHidden) {
  HiveParsedTx parsed;

...
static bool kkconfirm_sendTiny(uint16_t msgId, const uint8_t* payload,
  ...
  int port = 11044;
  if (const char* configured = getenv("KEEPKEY_UDP_PORT")) {
    const int candidate = atoi(configured);
...
TEST(Thorchain, ThorchainSignTxRejectsNullRecipient) {
  HDNode node = kSignNode;
  hdnode_fill_public_key(&node);

  ASSERT_TRUE(thorchain_signTxInit(&node, &kSignTx));
  EXPECT_FALSE(thorchain_signTxUpdateMsgSend(100000, nullptr, "rune"));
  thorchain_signAbort();
}
...
TEST(Thorchain, ConfirmThorTxAvaxLongMemoDecodesFully) {
  ...
  msg.value.size = 4;
  memcpy(msg.value.bytes, "\x3b\x9a\xca\x00", msg.value.size);

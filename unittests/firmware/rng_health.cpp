...
std::vector<uint8_t> pseudo16(size_t len, uint16_t seed) {
  std::vector<uint8_t> v(len);
  uint16_t s = seed;
  for (size_t i = 0; i < len; i++) {
    s = static_cast<uint16_t>(s * 25173u + 13849u);
    v[i] = static_cast<uint8_t>(s >> 8);
  }
  return v;
}
...
TEST(RngHealth, PassesTinySeedGeneratorByDesign) {
  auto v = pseudo16(RNG_HEALTH_SAMPLE_BYTES, 0xBEEF);
  EXPECT_TRUE(rng_health_analyze(v.data(), v.size()));
}
...
TEST(RngHealth, CheckedDrawFailsClosedWhenTheSourceStopsResponding) {
  rng_test_power_on_reset();
  rng_health_force_verdict(true);
  rng_test_force_bounded_failure(true);

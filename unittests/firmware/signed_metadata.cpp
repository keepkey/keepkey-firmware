...
TEST_F(SignedMetadataTest, WideKeyIdCannotAliasAValidSlot) {
  const std::vector<uint8_t> blob = base_blob();
  EXPECT_EQ(signed_metadata_process(blob.data(), blob.size(), 256u),
            METADATA_MALFORMED);
  EXPECT_FALSE(signed_metadata_available());
  EXPECT_EQ(signed_metadata_get(), nullptr);
}

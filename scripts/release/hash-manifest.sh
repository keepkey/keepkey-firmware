...
self_test() {
  ...
  # A descriptor with signatures but no application code is not firmware.
  printf '\001\002\004\001' | dd of="$e/firmware.keepkey.v0.0.0.bin" \
  ...
  printf '\000\000\000\000' | dd of="$e/firmware.keepkey.v0.0.0.bin" \
    bs=1 seek=4 conv=notrunc 2>/dev/null
  if sh "$0" --require-signed "$e" 0.0.0 full "" >/dev/null 2>&1; then
    rm -rf "$e"; fail "zero-length application passed signed gate"
  fi
  rm -rf "$e"
...
for f in *.bin *.elf; do
  ...
  if [ "$CODELEN" -eq 0 ]; then
    echo "ERROR: '$f' declares an empty application code region." >&2
    exit 1
  fi

...
SIGNATURES_C = REPO / "lib" / "board" / "signatures.c"
...
def load_key_policy(path=SIGNATURES_C):
    """Parse valid_pubkey[]; 0xff is active, matching signatures_ok()."""
    src = path.read_text()
    match = re.search(r"valid_pubkey\s*\[[^]]+\]\s*=\s*\{([^}]*)\}", src, re.S)
    if not match:
        raise ValueError("valid_pubkey[] policy did not parse")
    values = [int(value, 16) for value in re.findall(r"0x([0-9a-fA-F]{2})", match.group(1))]
    if not values:
        raise ValueError("valid_pubkey[] policy is empty")
    return [value == 0xff for value in values]


def verify_image(path, keys, active_keys=None):
    ...
    if active_keys is not None:
        if len(active_keys) != len(keys):
            return False, out + ["  signing-key policy length does not match pubkey table"]
        expired = [idx for idx in indices if not active_keys[idx - 1]]
        if expired:
            return False, out + [f"  expired signing key indices {expired}"]
...
def self_test():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        def check(label, blob, expect):
            p.write_bytes(blob)
            got, _ = verify_image(p, keys, [True] * len(keys))
            if got != expect:
        ...
        p.write_bytes(build([1, 2, 3]))
        got, _ = verify_image(p, keys, [False, True, True, True, True])
        if got:
            failures.append("an expired signing key was accepted")
...
def main(argv):
    ...
    for arg in argv[1:]:
        ...
        if not path.is_file():
            ...
            worst = 1
            continue
        ok, lines = verify_image(path, keys, active_keys)
        print("\n".join(lines))

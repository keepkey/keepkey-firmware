            ...
            steps {
                ...
                ansiColor('xterm') {
                    sh '''
                        ...
                        echo "Bootstrap Size, Bootloader Size (KeepKey), Firmware Size (KeepKey), Firmware Size (MFR), Variant Size (KeepKey), Variant Size (SALT), Variant Size (FOX), Variant Size (Blockpit), Variant Size (Kaspersky), Variant Size (Dash)" >> bin/binsize.csv
                        echo "$(du -b bin/bootstrap.bin | cut -f1), $(du -b bin/bootloader.bin | cut -f1), $(du -b bin/firmware.keepkey.bin | cut -f1), $(du -b bin/firmware.mfr.bin | cut -f1), $(du -b bin/variant.keepkey.bin | cut -f1), $(du -b bin/variant.salt.bin | cut -f1)", $(du -b bin/variant.fox.bin | cut -f1), $(du -b bin/variant.blockpit.bin | cut -f1), $(du -b bin/variant.kaspersky.bin | cut -f1), $(du -b bin/variant.dash.bin | cut -f1) >> bin/binsize.csv
